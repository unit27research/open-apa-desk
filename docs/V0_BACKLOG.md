...
- Resolve required APA page numbers for V0.
  - V0.1 status: starter-template copy path implemented and live-proven in
    Google Docs for page `1`, dynamic page `2`, setup preservation, and
    PDF/DOCX export preservation.
  - Required path: prepared APA starter template with export evidence.
- Add reference edit/delete controls in the sidebar.
  - V0.1 status: implemented and included in live smoke coverage.
- Add body-section insertion so users have a clean writing area after the title page.
  - V0.1 status: controlled body-title starter implemented.
...
- Add duplicate-reference detection.
  - V0.1 status: DOI and manual-entry duplicate collapse implemented.
...
- Confirm DOI lookup uses a project contact email before public DOI testing.
  - V0.1 status: runtime now uses the public Unit27 Research fallback when
    ...
    malformed or example `CROSSREF_MAILTO` overrides. Sidebar DOI lookup was
    included in live smoke coverage.
...
- Keep live smoke coverage current after any change to the reduced-scope
  `Prepare Current Copy` path.
